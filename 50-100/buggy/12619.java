...
public void loggingResponseFilterDoesntLogWhenSpecifiedMatcherIsNotFulfilled() throws java.lang.Exception {
    ...
    io.restassured.RestAssured.given().filter(io.restassured.filter.log.ResponseLoggingFilter.logResponseToIfMatches(captor, equalTo(400))).expect().body("greeting", equalTo("Greetings John Doe")).when().get("/greet?firstName=John&lastName=Doe");
