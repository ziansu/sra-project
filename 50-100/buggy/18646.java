public boolean userExists(java.lang.String username) throws pt.gov.dgarq.roda.common.convert.db.model.exception.ModuleException {
    boolean ret;
    try {
        java.sql.ResultSet result = getConnection(pt.gov.dgarq.roda.common.convert.db.modules.mySql.out.MYSQL_CONNECTION_DATABASE).createStatement().executeQuery((("SELECT User FROM mysql.user WHERE User =  '" + username) + "'"));
        ret = result.next();
