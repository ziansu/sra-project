...
public org.springframework.http.ResponseEntity<java.util.List<java.lang.String>> callInAnotherThread(java.security.Principal principal) throws java.lang.InterruptedException, java.util.concurrent.ExecutionException {
    ...
    java.util.concurrent.Future result = executor.submit(new com.github.bilak.poc.hystrix_oauth2_feign.api.rest.SampleController.SampleServiceClientCaller(sampleServiceClient));
    return java.util.Optional.ofNullable(result.get()).map(( r) -> org.springframework.http.ResponseEntity.ok(r)).orElse(new org.springframework.http.ResponseEntity(org.springframework.http.HttpStatus.NOT_FOUND));
}