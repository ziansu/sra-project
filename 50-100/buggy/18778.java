...
public xl.application.social.whatsup.util.Page<xl.application.social.whatsup.model.feed.entity.FeedEntry> list(int offset, int count) {
    java.util.List<xl.application.social.whatsup.model.feed.entity.FeedEntry> result = entries.descendingSet().stream().skip(offset).limit(count).collect(java.util.stream.Collectors.toList());
