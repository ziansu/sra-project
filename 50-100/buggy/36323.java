public static void registerShowDialog(java.lang.String title, java.lang.String type) {
    ...
    net.demilich.metastone.analytics.MetastoneAnalytics.INSTANCE.analyticsTracker.type(GoogleAnalytics.HitType.screenview).applicationVersion(BuildConfig.VERSION).category("dialog").action("show").screenName(title).label(type).build().send(false);
}