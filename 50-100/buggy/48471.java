...
private int getPositionTypeLinear(int itemPosition, int adapterPosition, boolean isReverseLayout) {
    boolean isFirstItem = (isReverseLayout) ? adapter.isLastItemInManager(adapterPosition) : itemPosition == 0;
    boolean isLastItem = (isReverseLayout) ? itemPosition == 0 : adapter.isLastItemInManager(adapterPosition);
