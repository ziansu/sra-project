...
public void onPlayerInteract(org.bukkit.event.player.PlayerInteractEvent event) {
    if (((event.getAction()) == (org.bukkit.event.block.Action.RIGHT_CLICK_BLOCK)) && ((event.getClickedBlock().getType()) == (org.bukkit.Material.ENCHANTMENT_TABLE))) {
        ...
        Enchantism.openInventories.put(thePlayer, new tk.thundaklap.enchantism.EnchantInventory(thePlayer, thePlayer.getTargetBlock(null, 500).getLocation(), tk.thundaklap.enchantism.Enchantism.getInstance().configuration.requireBookshelves));
    }
}