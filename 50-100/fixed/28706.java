...
public void loggingResponseFilterLogsNonErrors() throws java.lang.Exception {
    ...
    given().filter(io.restassured.filter.log.ResponseLoggingFilter.logResponseTo(captor)).expect().body("greeting", equalTo("Greetings John Doe")).when().get("/greet?firstName=John&lastName=Doe");
