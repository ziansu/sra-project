private com.netflix.discovery.shared.transport.jersey.JerseyEurekaHttpClientFactory buildLegacy() {
    com.netflix.discovery.shared.transport.jersey.EurekaJerseyClientImpl.EurekaJerseyClientBuilder clientBuilder = new com.netflix.discovery.shared.transport.jersey.EurekaJerseyClientImpl.EurekaJerseyClientBuilder().withClientName(clientName).withUserAgent("Java-EurekaClient").withConnectionTimeout(connectionTimeout).withReadTimeout(readTimeout).withMaxConnectionsPerHost(maxConnectionsPerHost).withMaxTotalConnections(maxTotalConnections).withConnectionIdleTimeout(((int) (connectionIdleTimeout))).withEncoderWrapper(encoderWrapper).withDecoderWrapper(decoderWrapper);
