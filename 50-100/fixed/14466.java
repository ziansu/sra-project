public static void registerDismissDialog(java.lang.String title, java.lang.String type, int dialogResult) {
    ...
    net.demilich.metastone.analytics.MetastoneAnalytics.INSTANCE.analyticsTracker.type(GoogleAnalytics.HitType.screenview).applicationVersion(BuildConfig.VERSION).category("dialog").action("dismiss").screenName(title).label(type).value(dialogResult).build().send();
}