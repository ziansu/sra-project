private void assignErrorTbl() {
    ...
    if ((_targetTdTableName.length()) <= (azkaban.jobtype.connectors.teradata.TdchParameters.ERROR_TABLE_NAME_LENGTH_LIMIT)) {
        ...
        azkaban.jobtype.connectors.teradata.TdchParameters._logger.info(((((((("Error table will be randomly decided by Teradata because " + (TdchConstants.ERROR_TABLE_KEY)) + " is not defined and ") + (TdchConstants.TARGET_TD_TABLE_NAME_KEY)) + " is longer than ") + (azkaban.jobtype.connectors.teradata.TdchParameters.ERROR_TABLE_NAME_LENGTH_LIMIT)) + " so that it cannot be used as a prefix of the error table. Please specify ") + (TdchConstants.ERROR_TABLE_KEY)));
    }
}