...
org.graylog2.indexer.searches.Sorting sorting) {
    return org.graylog2.indexer.searches.SearchesConfig.builder().query(query).filter(filter).fields(fields).range(timeRange).limit(limit).offset(offset).sorting(sorting).build();
}