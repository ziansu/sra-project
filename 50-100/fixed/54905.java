private boolean isEvictableFreeHeapPercentage(final com.hazelcast.map.impl.MapContainer mapContainer) {
    ...
    final float freeHeapPercentage = com.hazelcast.map.impl.eviction.MaxSizeChecker.getApproximateMaxSize(maxSizeConfig.getSize());
    final long total = getTotalMemory();
    return freeHeapPercentage > (((1.0F * (com.hazelcast.map.impl.eviction.MaxSizeChecker.ONE_HUNDRED_PERCENT)) * currentFreeHeapSize) / total);
}