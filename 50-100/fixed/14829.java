protected java.util.Set<uk.gov.gchq.gaffer.store.StoreTrait> fetchTraits() {
    ...
    for (final java.lang.String url : config.getUrls()) {
        final java.util.Set<uk.gov.gchq.gaffer.store.StoreTrait> traits = requestor.doGet(url, ("graph/" + (uk.gov.gchq.gaffer.federated.rest.service.IFederatedGraphConfigurationService.STORE_TRAITS_PATH)), new uk.gov.gchq.gaffer.store.TypeReferenceStoreImpl.StoreTraits(), null, true);
        if (null != traits) {
