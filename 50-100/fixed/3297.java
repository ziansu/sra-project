...
public seedu.task.logic.commands.CommandResult execute() {
    if (commandToShowUser.isEmpty()) {
        ...
        return new seedu.task.logic.commands.CommandResult(seedu.task.logic.commandlibrary.CommandLibrary.getInstance().getCommandUsage(commandToShowUser.trim().toLowerCase()));
    }
}