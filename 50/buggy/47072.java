@org.springframework.data.jpa.repository.Query(value = "SELECT e FROM Docsgeneral e inner join fetch e.exhibition ex WHERE (e.customer.id = :customerId or :customerId is null) and " + (((("(e.typeOfImport.id =:typeOfImport or :typeOfImport is null) and " + "e.typeOfDocs =:typeOfDocs and ") + "(e.typeOfContainer.id =:typeOfContainer or :typeOfContainer is null) and ") + "e.doAccounting =:doAccounting and ") + "(ex.devDate >= :startDate or :startDate is null) and (ex.devDate <= :endDate or :endDate is null) "))
...
java.lang.Boolean doAccounting, @org.springframework.data.repository.query.Param(value = "typeOfDocs")
com.vn.ael.enums.ServicesType typeOfDocs, @org.springframework.data.repository.query.Param(value = "startDate")
