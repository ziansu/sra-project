@org.springframework.data.jpa.repository.Query(value = "SELECT d FROM Duck d WHERE " + ((("LOWER(d.duckOwner.firstName)   LIKE LOWER(CONCAT(:ownerFirstName,'%')) AND " + "LOWER(d.duckOwner.lastName)    LIKE LOWER(CONCAT(:ownerLastName,'%'))  AND ") + "LOWER(d.duckBuyer.email)       LIKE LOWER(CONCAT(:email,'%'))          AND ") + "LOWER(d.duckOwner.phoneNumber) LIKE LOWER(CONCAT(:phone,'%'))"))
...
java.lang.String phone, org.springframework.data.domain.Pageable pageable);