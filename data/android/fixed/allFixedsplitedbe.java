private void showResult(java.lang.Integer message) {
    android.widget.Toast.makeText(this, message.toString(), Toast.LENGTH_LONG).show();
}
...
public void load(TModel model, com.raizlabs.android.dbflow.structure.database.DatabaseWrapper databaseWrapper) {
    getSingleModelLoader().load(databaseWrapper, com.raizlabs.android.dbflow.sql.language.SQLite.select().from(getModelClass()).where(getPrimaryConditionClause(model)).getQuery(), model);
}
...
public boolean isEmpty(android.widget.EditText string) {
    if ((string.getText().toString().length()) == 0)
        return true;
    else
        return false;
...
public void tellJoke(android.view.View view) {
    new com.shentuo.builditbigger.EndpointsAsyncTask().execute(this);
}
...
public void onCreate(android.os.Bundle savedInstanceState) {
    ...
    MinutesLeft = new java.lang.Long(0);
}
...
@java.lang.Override
public void loadUrl(final java.lang.String url, boolean clearNavigationStack, java.util.Map<java.lang.String, java.lang.String> headers) {
...
protected com.bitdubai.sub_app.intra_user_community.holders.AppNotificationsHolder createHolder(android.view.View itemView, int type) {
    return new com.bitdubai.sub_app.intra_user_community.holders.AppNotificationsHolder(itemView);
}
